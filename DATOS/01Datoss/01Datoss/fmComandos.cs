...
namespace _01Datoss {
    public partial class fmComandos : Form {
        ...
        private void teléfonosDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e) {
            //Para coger el nombre de un TIO
            lb1.Text = Convert.ToString(teléfonosDataGridView.Rows[e.RowIndex].Cells[1].Value);
        }
        ...
        private void MostrarPosicion() {
            //Total elementos
            int iTotal = teléfonosBindingSource.Count;
            int iPos;
            if(iTotal == 0) {
                laPosicion.Text = "0 de 0";
            } else {
                iPos = teléfonosBindingSource.Position + 1;
